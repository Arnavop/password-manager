    ...
    if len(website) == 0 or len(password) ==0:
        ...
        if is_ok:
            with open("data.txt","a") as data_file:
                data_file.write(f"{website.lower()} | {email} | {password}\n")
...
def find_passwd():
    ...
    if len(website) ==0:
        ...
        with open("data.txt","r") as data_file:
            ...
            for i in data:
                if website.lower() in i:
                    messagebox.showinfo(title="Password found",message=i)
                    break
            else:
                messagebox.showinfo(title="Error",message=f"No entry for {website} found in the file")
...
find_password = Button(text="Find Password",command=find_passwd)
find_password.grid(row=1,column=2)
